...
button_quitter.pack(padx=10, pady=10, anchor="e")
...
def valider():
    ...
    label_dt_result.config(
        text=f"D'après le DECISION TREE CLASSIFIER,\n{firstname} {lastname} devrait {result} (y_pred = {y_pred1})")
    result = "survivre" if y_pred2 == 1 else "mourir"
    label_knn_result.config(
        text=f"D'après le K NEIGHBORS CLASSIFIER,\n{firstname} {lastname} devrait {result} (y_pred = {y_pred2})")
    result = "survivre" if y_pred3f == 1 else "mourir"
    label_reg_result.config(
        text=f"D'après la Régréssion logistique,\n{firstname} {lastname} devrait {result} (y_pred = {y_pred3f})")


# -------------- H1 TITLE ---------------

title_label = tk.Label(root, text="Créez votre passager, et découvrez si il aurait survécu au naufrage du Titanic ?",
                       font=("Times", 24, "bold"))
...
input_lastname.insert(0, "DeWitt Bukater")
...
title_dt_metrics = tk.Label(metrics_frame, text="DECISION TREE CLASSIFIER", font=("TkDefaultFont", 10, "bold"),
                            anchor='center')
...
title_knn_metrics = tk.Label(metrics_frame, text="K NEIGHBORS CLASSIFIER", font=("TkDefaultFont", 10, "bold"),
                             anchor='center')
...
title_reg_metrics = tk.Label(metrics_frame, text="REGRESSION LOGISTIQUE", font=("TkDefaultFont", 10, "bold"),
                             anchor='center')
